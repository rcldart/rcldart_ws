...
class FoxgloveWsHub implements TopicSource {
  ...
  // ---- client publish (Foxglove clientPublish capability) -------------------
  int _nextClientChannel = 1;

  /// Advertise a channel this client will publish on (topic + ros2msg schema).
  /// Returns the client channel id to pass to [publishRaw]. The bridge must have
  /// the `clientPublish` capability (foxglove_bridge enables it by default).
  int advertiseClient(String topic, String schemaName, String schema) {
    final id = _nextClientChannel++;
    _ch?.sink.add(jsonEncode({
      'op': 'advertise',
      'channels': [
        {
  ...
  /// Publish one raw-CDR message on a previously [advertiseClient]-ed channel.
  /// Wire format: [0x01 opcode][uint32 LE channelId][cdr payload].
  void publishRaw(int clientChannelId, Uint8List cdr) {
    final frame = Uint8List(5 + cdr.length);
    frame[0] = 0x01; // ClientMessageData
    final bd = ByteData.sublistView(frame);
    bd.setUint32(1, clientChannelId, Endian.little);
    frame.setRange(5, 5 + cdr.length, cdr);
    _ch?.sink.add(frame);
  }
